...
<?php
    ...
    if (isset($_GET["universityId"])) /// testez daca este setata vreo universitate
    ...
    if (isset($_GET["studentId"])) /// testez daca este setat vreun student
    ...
    // checks if the student already has an active application
    $sqlApplication = "SELECT * FROM `applicationStatus` WHERE `universityId` = '$universityId' AND `studentId` = '$studentId'";
    $queryApplication = mysqli_query($link, $sqlApplication);

    if (mysqli_num_rows($queryApplication) > 0) {
        $applicationData = mysqli_fetch_assoc($queryApplication);
        $applicationId = $applicationData["applicationId"];
        $url = $base_url . "application.php?applicationId=" . $applicationId;
        $_SESSION["error"] = "Aplicatia deja exista! Editeaz-o de aici!";
        header("location: " . $url);
    }
    ...
    if (mysqli_num_rows($queryStudent) > 0) /// testez daca exista user cu id-ul respectiv (daca nu probabil exista un bug)
    ...
    if (!($studentData['consultantId'] == $accountId || $typeAccount == 1)) { //testez daca userul curent (admin / consultant) are acces la acest elev 
        header("location: index.php");
        die();
    }
    ...
    $sqlUniversity = "SELECT * FROM universities WHERE `universityId` = ".$universityId;
    $queryUniversity = mysqli_query($link, $sqlUniversity);
    ...
    if (mysqli_num_rows($queryUniversityData) > 0)/// testez daca exista universitatea cu id-ul respectiv 
    ...
    $statusArray = ["In progress","Accepted","Rejected","Waitlisted"];
    $nStatusArray = 4;
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sqlUpdate = "INSERT INTO `applicationStatus` (`studentId`, `universityId`, `appStatus`, `scholarship`) VALUES ('$studentId', '$universityId', '$status', '$scholarship')";
        mysqli_query($link, $sqlUpdate);

        $sqlApplication = "SELECT `applicationId` FROM `applicationStatus` WHERE `universityId` = '$universityId' AND `studentId` = '$studentId'";
        $queryApplication = mysqli_query($link, $sqlApplication);
        if (mysqli_num_rows($queryApplication) > 0) { // daca nu intra aici problem
            $applicationData = mysqli_fetch_assoc($queryApplication);
            $applicationId = $applicationData["applicationId"];

            // Add default university checklist items to applications_checklist
            $sqlUniversityChecklist = "SELECT checklistId FROM universities_checklist WHERE universityId = '$universityId'";
            $queryUniversityChecklist = mysqli_query($link, $sqlUniversityChecklist);
            while ($row = mysqli_fetch_assoc($queryUniversityChecklist)) {
                $checklistId = $row['checklistId'];
                $sqlInsertChecklist = "INSERT INTO applications_checklist (applicationId, checklistId, isCustom, status) VALUES ('$applicationId', '$checklistId', 0, 'In Progress')";
                mysqli_query($link, $sqlInsertChecklist);
            }

            $url = $base_url . "application.php?applicationId=" . $applicationData["applicationId"];
            header("location: " . $url);
            die();
        }
...
<html lang="en">
  ...
  <div id = "content">
    ...
    <h1 style = "color: rgba(79, 35, 95, .9);"> Add application for <?php echo $studentName; ?> at <?php echo $universityName;?>(<?php echo $universityCountry; ?>) </h1>
