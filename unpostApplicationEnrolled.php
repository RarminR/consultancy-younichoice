...
<?php
    ...
    // First, get the current postedEnrolledStatus
    $sqlGetCurrentStatus = "SELECT postedEnrolledStatus FROM applicationStatus WHERE applicationId = " . $applicationId;
    $result = mysqli_query($link, $sqlGetCurrentStatus);
    $currentStatus = mysqli_fetch_assoc($result)['postedEnrolledStatus'];
    ...
    // Update the postedEnrolledStatus
    $sqlPostApplication = "UPDATE applicationStatus SET postedEnrolledStatus = " . $newStatus . " WHERE `applicationId` = " . $applicationId;
    $queryApplicationData = mysqli_query($link, $sqlPostApplication);

    header("location: marketingEnrolledList.php");
?>
