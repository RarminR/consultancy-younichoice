...
if ($action === 'fetch') {
    ...
    if (count($applicationIds) > 0) {
        ...
        // Get all checklist items for these applications
        ...
        while ($row = mysqli_fetch_assoc($queryChecklist)) {
            ...
            if (!$isCustom) {
                ...
                if (!$resultActive) {
                    // If the query fails, assume the item is active (for backward compatibility)
                    $isActive = 1;
                } else {
                    $isActive = 0;
                    if ($rowActive = mysqli_fetch_assoc($resultActive)) {
        ...
        // For each checklist item, get all universities associated with it
        ...
        foreach ($checklistUnion as $checklistId => $checklistName) {
            $item = [
                ...
                'status' => $checklistIdToStatus[$checklistId] ?? 'In Progress',
                'isCustom' => (bool)($checklistIdToIsCustom[$checklistId] ?? 0),
                'universities' => $checklistIdToUniversities[$checklistId] ?? [],
                'hasDocument' => false, // For now, we'll set this to false since we're not checking documents
                'documentName' => null
            ];
            $checklistItems[] = $item;
        }
