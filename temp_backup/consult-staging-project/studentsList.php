  ...
  <div id = "content">
    ...
    <div id = "contentStudents">
        ...
        <ol id = "students-list" class="list-group list-group-numbered">
            ...
            if ($typeAccount == 1) {
                $sqlStudent = "SELECT * FROM studentData WHERE `consultantId` IN " . $consultantString ." AND `grade` IN ". $gradeString ." AND `packageType` IN " . $packageString . " AND `activityStatus` = 0";
                if (!empty($judetVariants)) {
                    $judetSqlList = "('" . implode("','", $judetVariants) . "')";
                    $sqlStudent .= " AND `judet` IN $judetSqlList";
                }
            } else {
                $sqlStudent = "SELECT * FROM studentData WHERE `consultantId` IN " . $consultantString ." AND `grade` IN ". $gradeString ." AND `packageType` IN " .$packageString . " AND `consultantId` = '$userId' AND `activityStatus` = 0";
                if (!empty($judetVariants)) {
                    $judetSqlList = "('" . implode("','", $judetVariants) . "')";
                    $sqlStudent .= " AND `judet` IN $judetSqlList";
                }
                    ...
                    <div class="list-group-item d-flex justify-content-between align-items-start">
                        <div class="ms-2 me-auto">
                            ...
                            <div class="student-info">
                                ...
                                <span><?php echo ($row['grade'] <= 12) ? $row['grade'] : 'Bachelor';?></span>
                            </div>
                            <div class="student-info">
    ...
    <script>
        ...
        // On page load: show sticky tags
        window.addEventListener('DOMContentLoaded', function() {
            if (selectedJudets && selectedJudets.length) {
                selectedJudets.forEach(j => addJudet(j));
            }
        });
    </script>
</body>
